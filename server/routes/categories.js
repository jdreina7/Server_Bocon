...
app.post('/category', auth.verifyToken, function (req, res) {
    ...
    let myCategory = new Category({
        ...
        cat_featured_img: body.cat_featured_img,
...
app.put('/category/:id', auth.verifyToken, function (req, res) {
    ...
    let body = _.pick(req.body, 
        ['cat_name', 'cat_description', 'cat_featured_img', 'cat_created_by'] );

	Category.findByIdAndUpdate( id, body, {new: true, runValidators: true}, (err, categoryDB2) => {
        if (err) {
            return res.status(400).json({
                ok: false,
                err
